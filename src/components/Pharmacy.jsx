...
export default function Pharmacy({ user, onComplete }) {
  ...
  const fetchPharmacyQueue = async () => {
    try {
      ...
      const { data: vsts } = await supabase.from('visits').select('*').eq('department', 'pharmacy');
  ...
  const allocateStockFEFO = (drugName, qtyNeeded) => {
    ...
    const drugBatches = batches.filter(b => b.name.toLowerCase() === drugName.toLowerCase());
    if (drugBatches.length === 0) return { allocated: [], status: 'out_of_stock' };

    // 2. Sort by expiry ascending (earliest expiring first)
    const sorted = [...drugBatches].sort((a, b) => new Date(a.expiry) - new Date(b.expiry));
    ...
    for (const b of sorted) {
      ...
      if (b.stock >= remainingNeeded) {
        allocated.push({ batch: b.batch, qty: remainingNeeded, expiry: b.expiry });
        remainingNeeded = 0;
        break;
      } else {
    ...
    if (remainingNeeded > 0) {
      return { allocated, status: 'partially_filled', remaining: remainingNeeded };
    }
  ...
  const handleDispense = async (orderId) => {
    if (!doubleChecked[orderId]) {
      setMessage({ type: 'error', text: 'Double-check verification required before release!' });
      return;
    }
    ...
    try {
      ...
      if (allocation.status === 'out_of_stock') {
        throw new Error(`Medication ${drugName} is currently out of stock in inventory!`);
      }
      ...
      const dispenseMeta = {
        dispensed_batch: allocation.allocated.map(a => `${a.batch} (${a.qty}u)`).join(', '),
      ...
      const allDispensed = updatedOrders.every(o => o.status === 'dispensed' || o.status === 'cancelled');
      ...
      if (allDispensed) {
        ...
        setMessage({ type: 'success', text: 'All prescriptions dispensed. Visit successfully completed!' });
        setTimeout(() => {
          fetchPharmacyQueue();
          if (onComplete) onComplete();
        }, 1500);
      } else {
        setMessage({ type: 'success', text: `Successfully dispensed ${drugName}.` });
  ...
  const handleRestock = () => {
    ...
    localStorage.setItem('egesa_pharmacy_batches', JSON.stringify(defaultBatches));
  ...
  const getAggregatedFormulary = () => {
    ...
    batches.forEach(b => {
      if (!map[b.name]) {
        map[b.name] = { name: b.name, totalStock: 0, unit: b.unit, isLow: false, nearExpiryBatches: [] };
      }
            ...
            >
              <div className="flex justify-between items-center w-full">
                ...
                <span className={`text-[8px] px-1 rounded uppercase font-bold ${
                  item.priority === 'emergency' ? 'bg-red-500/10 text-red-400 border border-red-500/25 animate-pulse' : 'bg-slate-850 text-slate-400'
                }`}>{item.priority}</span>
              </div>
      ...
      <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
        {!selectedVisit ? (
          <div className="flex flex-col items-center justify-center py-32 text-center my-auto">
            ...
            <h3 className="text-slate-400 font-bold text-sm uppercase tracking-wide">No Prescription Loaded</h3>
            <p className="text-slate-600 text-xs max-w-xs mt-1">Select a patient from the pharmacy queue to review clinical suitability, inspect batches, and double-check dispensing records.</p>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between md:items-center pb-4 border-b border-slate-800 gap-4">
              <div>
                <span className="text-xs text-teal-400 font-bold uppercase tracking-wider">Clinical Prescriptions Dispensary Desk</span>
            ...
            <div className="space-y-4">
              <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider">Prescribed Meds & FEFO Stock Checker</h4>
              ...
              <div className="space-y-4">
                {pendingPrescriptions.map((presc) => {
                  ...
                  return (
                    <div key={presc.id} className="bg-slate-950 border border-slate-850 p-4 rounded-xl space-y-3">
                      <div className="flex justify-between items-start gap-4">
                        <div>
                          <span className="font-bold text-slate-200 text-xs block">{drugName}</span>
                          <p className="text-[10px] text-slate-500 font-medium">Sig: {presc.instructions}</p>
                        </div>
                        <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase border ${
                          isDispensed ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                          state === 'approved' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                          state === 'cancelled' ? 'bg-red-500/10 text-red-400 border-red-500/20' :
                          ...
                          {allocation.status === 'out_of_stock' ? (
                            ...
                            <div className="space-y-1 pl-3 font-mono text-[9px] text-slate-400">
                              {allocation.allocated.map((a, i) => (
                                <p key={i}>
                                  Deduct <span className="text-white font-bold">{a.qty}u</span> from Batch <span className="text-teal-400 font-bold">{a.batch}</span> (expires {a.expiry})
                                </p>
                              ))}
                              {allocation.status === 'partially_filled' && (
                                <p className="text-yellow-450 font-bold">
                                  Warning: Partially filled. Deficit of {allocation.remaining} units.
                                </p>
                              )}
                            </div>
                          )}
                      ...
                      {isDispensed && (
                        <div className="bg-slate-900/60 p-2.5 rounded border border-slate-850/50 text-[10px] space-y-1">
                          <p className="text-slate-400">Dispensed Batch: <span className="font-mono text-slate-200">{parseOrderMeta(presc.results).dispensed_batch}</span></p>
                          <p className="text-slate-500 text-[9px]">Dispensed by {parseOrderMeta(presc.results).dispensed_by} at {new Date(parseOrderMeta(presc.results).dispensed_at).toLocaleString()}</p>
                        </div>
                      )}
                      ...
                      {!isDispensed && (
                        <div className="border-t border-slate-900/60 pt-3 flex flex-col md:flex-row md:justify-between md:items-center gap-3">
                          <div className="flex gap-2">
                            ...
                            {state !== 'prescribed' && state !== 'dispensed' && (
                              <button
                                type="button"
                                onClick={() => handleReviewStatus(presc.id, 'prescribed')}
                          ...
                          {state === 'approved' && allocation.status !== 'out_of_stock' && (
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm space-y-4 lg:col-span-1">
        <div className="flex justify-between items-center pb-2 border-b border-slate-800">
          <div>
            ...
            <p className="text-[10px] text-slate-550 mt-0.5">FEFO Stock Lot Rotations</p>
          </div>
          <button
            ...
            <div key={idx} className="bg-slate-950 border border-slate-850 p-2.5 rounded-xl space-y-2">
              <div className="flex justify-between items-start gap-1">
                ...
                <span className={`font-mono text-[10px] font-bold px-1.5 py-0.2 rounded shrink-0 ${
                  item.isLow ? 'text-red-400 bg-red-500/5 border border-red-500/15' : 'text-slate-405 bg-slate-850'
                }`}>
              ...
              <div className="space-y-1 border-t border-slate-900 pt-1.5">
                {batches.filter(b => b.name === item.name).map((b, bIdx) => (
                  <div key={bIdx} className="flex justify-between text-[9px] text-slate-500 font-mono">
                    ...
                    <span className={isNearExpiry(b.expiry) ? 'text-red-400 font-bold' : 'text-slate-400'}>
