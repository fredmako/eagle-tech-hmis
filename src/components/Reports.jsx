...
export default function Reports({ user }) {
  ...
  const [facilityInfo, setFacilityInfo] = useState({
    ...
    code: "EMC-001",
    logo_url: null,
    address: "Avenue Rd, Nairobi, Kenya",
  });

  const renderFacilityLogo = (
    logoUrl,
    sizeClass = "h-8 w-8",
    textClass = "text-xs",
  ) => {
    if (!logoUrl) {
      return (
        <div
          className={`${sizeClass} rounded-lg bg-teal-500/10 border border-teal-500/30 flex items-center justify-center shrink-0 font-bold text-teal-600 ${textClass}`}
        >
    ...
    if (logoUrl.startsWith("preset:")) {
      ...
      if (presetKey === "shield") {
        return (
          <div
            className={`${sizeClass} rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-400 p-1 flex items-center justify-center shrink-0`}
          >
            <svg
      ...
      if (presetKey === "cross") {
        return (
          <div
            className={`${sizeClass} rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-400 p-1 flex items-center justify-center shrink-0`}
          >
            <svg
      ...
      return (
        <div
          className={`${sizeClass} rounded-lg bg-teal-500/10 border border-teal-500/20 text-teal-400 p-1 flex items-center justify-center shrink-0`}
        >
          <svg
  ...
  const forceSync = async () => {
    ...
    await supabase.from("audit_logs").insert({
      ...
      details:
        "Forced data upload to DHIS2. Cleaned records compiled successfully.",
    });
    ...
    try {
      await sendNotification(
        ...
        {
          details:
            "Forced sync check override to upload clinic records to DHIS2.",
  ...
  const checkReportAccess = (reportId) => {
    ...
    return (
      reportPermissions[reportId]?.some((r) => rolesList.includes(r)) || false
    );
  };
    ...
    switch (reportId) {
      case "reg_daily": {
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Patients Registered: ${filtered.length}`,
            right: "",
          },
        };
      }
      case "clinical_encounter": {
        const filtered = visits.filter(
          (v) => isInRange(v.created_at) && v.department !== "ward",
        );
        const headers = [
          "Date",
          "Patient Name",
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Consultations Completed: ${filtered.length}`,
            right: "",
          },
        };
      }
      case "lab_order": {
        const filtered = orders.filter(
          (o) => o.type === "lab" && isInRange(o.created_at),
        );
        const headers = [
          "Date Ordered",
          "Patient Name",
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Tests Ordered: ${filtered.length}`,
            right: `Pending: ${filtered.filter((o) => o.status !== "completed").length}`,
          },
        };
      }
      case "lab_results": {
        ...
        const rows = filtered.map((o) => {
          ...
          if (o.results) {
            if (o.results.startsWith("{")) {
              try {
                resVal = JSON.parse(o.results).values || "—";
              } catch (e) {}
            } else {
              resVal = o.results;
            }
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Results Released: ${filtered.length}`,
            right: "",
          },
        };
      }
      case "pharm_prescription": {
        const filtered = orders.filter(
          (o) => o.type === "prescription" && isInRange(o.created_at),
        );
        const headers = [
          "Date",
          "Patient Name",
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Prescriptions Logged: ${filtered.length}`,
            right: "",
          },
        };
      }
      case "pharm_dispense": {
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Items Dispensed: ${filtered.length}`,
            right: "",
          },
        };
      }
      case "pharm_revenue": {
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Pharmacy Sales: ${filtered.length}`,
            right: `Estimated Revenue: KES ${totalRev.toFixed(2)}/-`,
          },
        };
      }
      case "pharm_stock": {
        ...
        const rows = batches.map((b) => {
          ...
          const isWarning =
            new Date(b.expiry) <
              new Date(Date.now() + 60 * 24 * 60 * 60 * 1000) && !isExpired;
          const expStatus = isExpired
            ? "Expired"
            : isWarning
              ? "Expiring Soon"
              : "Valid";
          return [b.batch, b.name, `${b.stock} ${b.unit}`, b.expiry, expStatus];
        });
        return {
          ...
          rows,
          totals: {
            left: `Total Batches Tracked: ${batches.length}`,
            right: "",
          },
        };
      }
      case "billing_receipts": {
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Invoices: ${filtered.length}`,
            right: `Billed: KES ${totalAmount.toFixed(2)} | Paid: KES ${totalPaid.toFixed(2)}`,
          },
        };
      }
      case "ward_admissions": {
        const filtered = visits.filter(
          (v) => v.department === "ward" && isInRange(v.created_at),
        );
        const headers = [
          "Admission Date",
          "Patient Name",
        ...
        return {
          ...
          rows,
          totals: {
            left: `Ward Admissions in period: ${filtered.length}`,
            right: `Active Bed Occupancy: ${filtered.filter((v) => v.status !== "completed").length}`,
          },
        };
      }
      default:
  ...
  const getComplianceReportData = (reportId) => {
    ...
    const isRevisit = (patientId, beforeDateStr) => {
      ...
      const priorVisits = visits.filter(
        (v) =>
          v.patient_id === patientId && new Date(v.created_at) < beforeDate,
      );
    ...
    switch (reportId) {
      case "moh_daily_under5": {
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Child Records: ${rows.length}`,
            right: "Compliance: MOH-204A Compliant",
          },
        };
      }
      case "moh_daily_over5": {
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Over 5 Records: ${rows.length}`,
            right: "Compliance: MOH-204B Compliant",
          },
        };
      }
      case "moh_anc": {
        ...
        patients.forEach((p) => {
          if (p.gender === "female") {
            ...
            if (isPreg) {
              const ptVisits = visits.filter(
                (v) => v.patient_id === p.id && isInRange(v.created_at),
              );
              ptVisits.forEach((v, idx) => {
                ...
                const gestationalWeeks = lmpDate
                  ? Math.round(
                      (new Date(v.created_at) - new Date(lmpDate)) /
                        (7 * 24 * 3600000),
                    )
                  : 20;
        ...
        return {
          ...
          rows: ancRows,
          totals: {
            left: `Total ANC Registrants: ${ancRows.length}`,
            right: "Compliance: MOH-405 Compliant",
          },
        };
      }
      case "moh_fp": {
        ...
        patients.forEach((p) => {
          ...
          ptVisits.forEach((v) => {
            ...
            const isFp =
              c?.diagnosis_icd10?.includes("Family Planning") ||
              ords.some(
                (o) =>
                  o.item_name.includes("DMPA") ||
                  o.item_name.includes("Contraceptive"),
              );
        ...
        return {
          ...
          rows: fpRows,
          totals: {
            left: `Total FP Clients: ${fpRows.length}`,
            right: "Compliance: MOH-512 Compliant",
          },
        };
      }
      case "moh_lab": {
        ...
        const rows = labOrders.map((o) => {
          ...
          return [
            ...
            meta.specimen_type ||
              (o.item_name.includes("Urinalysis") ? "Urine" : "Blood"),
            meta.specimen_condition || "Good",
            o.item_name,
            meta.collected_at
              ? new Date(meta.collected_at).toLocaleDateString()
              : new Date(o.created_at).toLocaleDateString(),
            meta.processing_started_at
              ? new Date(meta.processing_started_at).toLocaleDateString()
              : new Date(o.created_at).toLocaleDateString(),
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Lab Investigations: ${rows.length}`,
            right: "Compliance: MOH-240 Compliant",
          },
        };
      }
      case "moh_705a": {
        const childVisits = visits.filter((v) => {
          const p = patients.find((pt) => pt.id === v.patient_id);
          return (
            isInRange(v.created_at) && p && getPatientAgeInMonths(p.dob) < 60
          );
        });

        const diarrhoea = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Gastroenteritis") &&
            childVisits.some((v) => v.id === c.visit_id),
        ).length;
        const malaria = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Malaria") &&
            childVisits.some((v) => v.id === c.visit_id),
        ).length;
        const pneumonia = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Pneumonia") &&
            childVisits.some((v) => v.id === c.visit_id),
        ).length;
        const tonsillitis = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Tonsillitis") &&
            childVisits.some((v) => v.id === c.visit_id),
        ).length;
        const uti = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Urinary") &&
            childVisits.some((v) => v.id === c.visit_id),
        ).length;
        const totalCases = childVisits.filter((v) =>
          consultations.some((c) => c.visit_id === v.id),
        ).length;
        const others = Math.max(
          0,
          totalCases - (diarrhoea + malaria + pneumonia + tonsillitis + uti),
        );

        const headers = [
          "MOH Disease Classification",
          "Child Cases Count (< 5 yrs)",
          "Surveillance Code",
        ];
        const rows = [
          [
            "Diarrhoea / Gastroenteritis (ICD-10: A09)",
            diarrhoea,
            "MOH-705A-01",
          ],
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Outpatient Child Diagnoses: ${totalCases}`,
            right: "Compliance: DHIS2 Ready",
          },
        };
      }
      case "moh_705b": {
        ...
        const diarrhoea = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Gastroenteritis") &&
            adultVisits.some((v) => v.id === c.visit_id),
        ).length;
        const malaria = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Malaria") &&
        ...
        const hypertension = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Hypertension") &&
            adultVisits.some((v) => v.id === c.visit_id),
        ).length;
        const totalCases = adultVisits.filter((v) =>
          consultations.some((c) => c.visit_id === v.id),
        ).length;
        const others = Math.max(
          0,
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Outpatient Adult Diagnoses: ${totalCases}`,
            right: "Compliance: DHIS2 Ready",
          },
        };
      }
      case "moh_daily": {
        ...
        const rows = filtered.map((v, i) => {
          ...
          return [
            ...
            c?.diagnosis_icd10 || "—",
            t
              ? `BP: ${t.blood_pressure || (t.systolic ? `${t.systolic}/${t.diastolic}` : "—")}, Temp: ${t.temperature}°C, Weight: ${t.weight}kg`
              : "—",
          ];
        });
        return {
          ...
          rows,
          totals: {
            left: `Total Registered Records: ${filtered.length}`,
            right: "Compliance: DHIS2 Ready",
          },
        };
      }
      case "moh_monthly": {
        ...
        const periodAdmissions = periodVisits.filter(
          (v) => v.department === "ward",
        ).length;
        const periodOutpatients = periodVisits.filter(
          (v) => v.department !== "ward",
        ).length;

        const malaria = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Malaria") && isInRange(c.created_at),
        ).length;
        const respiratory = consultations.filter(
          (c) =>
            (c.diagnosis_icd10?.includes("Respiratory") ||
              c.diagnosis_icd10?.includes("Tonsillitis")) &&
            isInRange(c.created_at),
        ).length;
        const gastro = consultations.filter(
          (c) =>
            (c.diagnosis_icd10?.includes("Gastroenteritis") ||
              c.diagnosis_icd10?.includes("Amoebiasis")) &&
            isInRange(c.created_at),
        ).length;
        const hyper = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Hypertension") &&
            isInRange(c.created_at),
        ).length;
        const uti = consultations.filter(
          (c) =>
            c.diagnosis_icd10?.includes("Urinary") && isInRange(c.created_at),
        ).length;

        const totalInvs = invoices.filter((inv) => isInRange(inv.created_at));
        const totalRevenue = totalInvs.reduce(
          (acc, i) => acc + parseFloat(i.total_amount || 0),
          0,
        );

        const totalLab = orders.filter(
          (o) => o.type === "lab" && isInRange(o.created_at),
        ).length;
        const totalDisp = orders.filter(
          (o) =>
            o.type === "prescription" &&
            o.status === "approved" &&
            isInRange(o.created_at),
        ...
        const headers = [
          "MOH Indicator Metric",
          "Indicator Value",
          "MOH 717 Reporting Target Code",
        ];
        const rows = [
          ...
          [
            "Acute Respiratory Infections (ICD-10: J06.9)",
            respiratory,
            "MOH-717-ARI",
          ],
          [
            "Gastroenteritis / Diarrhoeal Diseases (ICD-10: A09)",
            gastro,
            "MOH-717-GDD",
          ],
          ...
          [
            "Aggregate Financial Revenue Billed",
        ...
        return {
          ...
          rows,
          totals: {
            left: `Indicators Compiled: 10`,
            right: "Ready for Ministry Submission Portal",
          },
        };
      }
      case "audit_logs": {
        ...
        const rows = auditLogs
          .filter((log) => isInRange(log.created_at))
          .map((log) => [
        ...
        return {
          ...
          rows,
          totals: {
            left: `Total Audit Entries: ${rows.length}`,
            right: "Classification: Restricted",
          },
        };
      }
      default:
  ...
  const handleExportReport = async (reportId, format, reportType) => {
    ...
    const report =
      reportType === "dept"
        ? getDepartmentReportData(reportId)
        : getComplianceReportData(reportId);
    ...
    if (format === "json") {
      ...
      fileContent =
        ...
        encodeURIComponent(JSON.stringify(dataArr, null, 2));
      filename += ".json";
    } else {
      ...
      fileContent =
        ...
        encodeURIComponent(csvHeaders + csvRows);
      filename += ".csv";
    }
      ...
      "REPORT_GENERATED",
      {
        ...
        recipientEmail:
          brandingMode === "platform"
            ? "info@eagletechsolutions.tech"
            : `info@${facilityInfo.name.toLowerCase().replace(/[^a-z0-9]/g, "")}.com`,
      },
      user.facility_id,
    );
  ...
  const handleGenerateReport = async () => {
    ...
    const handleAiWriteReport = async () => {
      ...
      try {
        ...
        const payload = {
          mode: "report",
          prompt: `Write a professional ${reportCategory} healthcare operations report summary for the period ${startDate} to ${endDate}. Include an executive overview, key metrics interpretation, notable trends, and recommended follow-up actions.`,
          context: {
            reportCategory,
            startDate,
            endDate,
        ...
        setAiReportResult(data.content || data.response || data);
      } catch (err) {
    ...
    try {
      ...
      if (reportFormat === "json") {
        ...
        fileContent =
          "data:text/json;charset=utf-8," +
          encodeURIComponent(JSON.stringify(trimmedData, null, 2));
        filename += ".json";
      } else {
        const csvHeaders =
          activeFields.map((f) => f.toUpperCase().replace("_", " ")).join(",") +
          "\n";
        const csvRows = filteredRecords
          .map((rec) => {
            return activeFields
              .map((f) => `"${String(rec[f] || "").replace(/"/g, '""')}"`)
              .join(",");
          })
          .join("\n");
        fileContent =
  ...
  const malariaCount = consultations.filter((c) =>
    c.diagnosis_icd10?.includes("Malaria"),
  ).length;
  const respiratoryCount = consultations.filter(
    (c) =>
      c.diagnosis_icd10?.includes("Respiratory") ||
      c.diagnosis_icd10?.includes("Tonsillitis"),
  ).length;
  const gastroCount = consultations.filter(
    (c) =>
      c.diagnosis_icd10?.includes("Gastroenteritis") ||
      c.diagnosis_icd10?.includes("Amoebiasis"),
  ).length;
  const hyperCount = consultations.filter((c) =>
    c.diagnosis_icd10?.includes("Hypertension"),
  ).length;
  const utiCount = consultations.filter((c) =>
    c.diagnosis_icd10?.includes("Urinary"),
  ).length;
  ...
  const customActiveColumns = Object.keys(customFieldsConfig).filter(
    (f) => customFieldsConfig[f],
  );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Access compliance structures, department ledgers, and manage
        ...
        <div className="bg-teal-500/10 border border-teal-500/35 text-teal-400 text-xs p-4 rounded-xl flex items-center gap-2.5 shadow animate-pulse">
          ...
          <div>
            <span className="font-bold block uppercase tracking-wider text-[9px] mb-0.5">
              Automated Task Dispatch Scheduled
            </span>
            <span>
              The report <strong>"{scheduledReportName}"</strong> is scheduled
              for daily automated email compilation and Titan SMTP dispatch at
              08:00 AM EAT.
            </span>
          </div>
        </div>
