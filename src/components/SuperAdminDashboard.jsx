...
const getRiskLabel = (row) => {
  ...
  if (row.triageComplianceRate < 60 && row.consultVisits > 0)
    return "Triage bypass";
  if (row.digitalPaymentRate < 25 && row.paidInvoices > 0)
    return "Low digital pay";
...
const buildClientOperationsRows = ({
  facilities,
  supportTickets,
  systemStats,
  rangeDays,
  ...
  return facilities.map((facility) => {
    ...
    const visits = systemStats.visits.filter(
      (v) =>
        v.facility_id === facilityId && isWithinDays(v.created_at, rangeDays),
    );
    const allVisits = systemStats.visits.filter(
      (v) => v.facility_id === facilityId,
    );
    const orders = systemStats.orders.filter(
      (o) =>
        o.facility_id === facilityId && isWithinDays(o.created_at, rangeDays),
    );
    const invoices = systemStats.invoices.filter(
      (i) =>
        i.facility_id === facilityId && isWithinDays(i.created_at, rangeDays),
    );
    const profiles = systemStats.profiles.filter(
      (p) => p.facility_id === facilityId,
    );
    const tickets = supportTickets.filter((t) => t.facility_id === facilityId);
    const consultVisits = visits.filter((v) =>
      v.department?.toLowerCase().includes("consult"),
    ).length;
    const triageVisits = visits.filter(
      (v) => v.department?.toLowerCase() === "triage",
    ).length;
    const labOrders = orders.filter(
      (o) =>
        o.type?.toLowerCase() === "lab" ||
        o.type?.toLowerCase() === "laboratory",
    );
    const verifiedLabOrders = labOrders.filter((o) =>
      ["authorized", "verified", "completed"].includes(o.status),
    );
    const paidInvoices = invoices.filter((i) => i.status === "paid");
    const digitalPayments = paidInvoices.filter((i) =>
      ["mpesa", "stk", "paypal", "card"].includes(
        i.payment_method?.toLowerCase(),
      ),
    );
    const pendingOrders = orders.filter(
      (o) =>
        !["completed", "dispensed", "verified", "authorized"].includes(
          o.status,
        ),
    ).length;
        ...
        (sum, invoice) =>
          ...
          Math.max(
            ...
            parseAmount(invoice.total_amount) -
              parseAmount(invoice.amount_paid),
          ),
        0,
      );
    const settledRevenue = paidInvoices.reduce(
      (sum, invoice) => sum + parseAmount(invoice.amount_paid),
      0,
    );
    const openSupport = tickets.filter((t) => t.status === "pending").length;
    const triageComplianceRate =
      consultVisits > 0
        ? Math.min(100, Math.round((triageVisits / consultVisits) * 100))
        : 100;
    const labVerificationRate =
      labOrders.length > 0
        ? Math.round((verifiedLabOrders.length / labOrders.length) * 100)
        : 100;
    const digitalPaymentRate =
      paidInvoices.length > 0
        ? Math.round((digitalPayments.length / paidInvoices.length) * 100)
        : 100;
    const receivablePenalty =
      pendingReceivables > 0
        ? Math.min(20, Math.log10(pendingReceivables + 1) * 4)
        : 0;
...
export default function SuperAdminDashboard({ user, onSignOut, onLogoClick }) {
  ...
  const handleResolveSupportTicket = async (ticketId) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[Support Reply Email] Notification skipped/failed:",
          mailErr,
        );
      }
      ...
      setMessage({
        type: "success",
        text: `Support ticket #${ticketId.substring(7, 13)} resolved and response sent successfully!`,
      });
    } catch (err) {
      console.error("Resolve ticket failed:", err);
      setMessage({
        type: "error",
        text: err.message || "Failed to resolve support ticket.",
      });
    } finally {
  ...
  const rangeDays =
    operationsState.range === "7d"
      ? 7
      : operationsState.range === "90d"
        ? 90
        : 30;

  const clientOperationsRows = useMemo(
    () =>
      buildClientOperationsRows({
        facilities,
        supportTickets,
        systemStats,
        rangeDays,
  ...
  const filteredClientOperationsRows = useMemo(() => {
    ...
    return clientOperationsRows
      .filter(
        (row) =>
          operationsState.tier === "all" ||
          row.licenseTier === operationsState.tier,
      )
      .filter((row) => {
        ...
        if (operationsState.status === "risk")
          return row.healthScore < 65 || row.openSupport > 0;
        return true;
      })
      .filter(
        (row) =>
          !query ||
          row.facilityName.toLowerCase().includes(query) ||
          row.code?.toLowerCase().includes(query),
      )
      .sort((a, b) => {
        if (operationsState.sort === "revenue_desc")
          return b.settledRevenue - a.settledRevenue;
        if (operationsState.sort === "visits_desc") return b.visits - a.visits;
        if (operationsState.sort === "support_desc")
          return b.openSupport - a.openSupport;
        if (operationsState.sort === "name_asc")
  ...
  const selectedOperationsRow = useMemo(
    () =>
      clientOperationsRows.find(
        (row) => row.facilityId === operationsState.facility,
      ) ||
      filteredClientOperationsRows[0] ||
      null,
    [
      clientOperationsRows,
      filteredClientOperationsRows,
      operationsState.facility,
    ],
  );
  ...
  const tierVerificationChart = useMemo(() => {
    ...
    return tiers.map((tier) => {
      const rows = clientOperationsRows.filter(
        (row) => row.licenseTier === tier,
      );
      return {
  ...
  const operationsSummary = useMemo(
    () => ({
      atRisk: clientOperationsRows.filter(
        (row) => row.healthScore < 65 || row.openSupport > 0,
      ).length,
      openSupport: clientOperationsRows.reduce(
        (sum, row) => sum + row.openSupport,
        0,
      ),
      pendingReceivables: clientOperationsRows.reduce(
        (sum, row) => sum + row.pendingReceivables,
        0,
      ),
      settledRevenue: clientOperationsRows.reduce(
        (sum, row) => sum + row.settledRevenue,
        0,
      ),
    }),
    [clientOperationsRows],
  ...
  useEffect(() => {
    ...
    if (activeTab === "insights") {
      ...
      if (operationsState.facility)
        params.set("facility", operationsState.facility);
    ...
    try {
      ...
      try {
        const { data: allProfiles } = await supabase
          .from("profiles")
          .select(
            "id, full_name, email, role, facility_id, access_status, created_at",
          );
      ...
      setMessage({
        type: "error",
        text: err.message || "Failed to load system control records.",
      });
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleUpdateDemoStatus = async (id, newStatus) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: `Demo prospect status updated to ${newStatus} successfully!`,
      });
    } catch (err) {
  ...
  const getDiagnosticRecommendations = () => {
    ...
    const triageCount = systemStats.visits.filter(
      (v) => v.department?.toLowerCase() === "triage",
    ).length;
    const consultCount = systemStats.visits.filter((v) =>
      v.department?.toLowerCase().includes("consult"),
    ).length;
    if (consultCount > 0 && triageCount / consultCount < 0.5) {
      recs.push({
        ...
        severity: "warning",
        description:
          "Fewer triage records than clinical consultation logs are registered. Facilities may be bypassing nurse vitals. Suggest sending a recommendation to enforce triage entry before consultation routing.",
        action: "Notify Facility Admins",
      });
    }

    // Rule 2: Automatic lab pathology verification
    const labOrders = systemStats.orders.filter(
      (o) =>
        o.type?.toLowerCase() === "lab" ||
        o.type?.toLowerCase() === "laboratory",
    );
    const pathVerified = labOrders.filter(
      (o) => o.status === "authorized" || o.status === "verified",
    ).length;
    if (labOrders.length > 5 && pathVerified / labOrders.length < 0.4) {
      recs.push({
        ...
        severity: "danger",
        description:
          'More than 60% of laboratory test runs remain in "technician draft" state without Pathologist authorization sign-off. Recommend contacting facility admins to verify pathology delegation roles.',
        action: "Review Lab Staff Roles",
      });
    }

    // Rule 3: Subdomain & White-Label Setup
    const standardOrElite = facilities.filter(
      (f) => f.license_tier === "standard" || f.license_tier === "extensive",
    );
    const unconfiguredSubdomains = standardOrElite.filter(
      (f) => !f.code || f.code.includes("EMC"),
    );
    ...
    const mobilePayments = systemStats.invoices.filter(
      (i) =>
        i.payment_method?.toLowerCase() === "mpesa" ||
        i.payment_method?.toLowerCase() === "stk",
    ).length;
    const totalInvPaid = systemStats.invoices.filter(
      (i) => i.status === "paid",
    ).length;
    if (totalInvPaid > 0 && mobilePayments / totalInvPaid < 0.2) {
      recs.push({
        ...
        severity: "info",
        description:
          "Cash accounts for over 80% of settled patient checkout invoices. Recommend promoting mobile payment integrations (M-Pesa STK Push / PayPal checkout portals) to reduce clinic cashier overhead.",
    ...
    if (recs.length === 0) {
      recs.push({
        ...
        severity: "success",
        description:
          "All system metrics (triage compliance, billing cycles, pathologist verifications, and custom subdomains) fall within optimal parameters.",
  ...
  const handleToggleVerification = async (facilityId, currentStatus) => {
    ...
    try {
      ...
      setFacilities((prev) =>
        prev.map((f) =>
          f.id === facilityId ? { ...f, is_verified: newStatus } : f,
        ),
      );
      setMessage({
        type: "success",
      ...
      setMessage({
        type: "error",
        text: err.message || "Failed to update facility verification.",
      });
    } finally {
      setActionLoading(null);
    }
  ...
  const handleAccessPortal = () => {
    const targetFacility =
      facilities.find((f) => f.id === "f1") || facilities[0];
    if (!targetFacility) {
  ...
  return (
    <div className="min-h-screen bg-slate-955 text-slate-100 flex flex-col font-sans">
      ...
      <header className="bg-linear-to-r from-slate-900 via-slate-900 to-slate-800 border-b border-slate-700/50 px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4 shadow-2xl backdrop-blur-sm">
        <div
          onClick={onLogoClick}
          className="flex items-center gap-3 cursor-pointer hover:opacity-90 transition-all duration-300 group"
        >
          <div className="bg-linear-to-br from-teal-500/20 to-teal-600/10 border border-teal-500/30 p-2 rounded-2xl shadow-lg shadow-teal-500/10 group-hover:shadow-teal-500/20 transition-all duration-300">
            <img
              src="/logo.png"
              alt="Eagle Tech Logo"
          ...
          <div>
            <h1 className="text-lg font-black tracking-wider text-white uppercase leading-tight">
              Eagle Tech Systems Control
            </h1>
            <span className="text-2xs text-teal-400 font-bold uppercase tracking-widest mt-0.5 bg-teal-500/10 px-2 py-0.5 rounded-full inline-block">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-left leading-none hidden sm:block">
              ...
              <span className="text-[9px] text-teal-500 font-semibold uppercase tracking-wider block mt-0.5">
                SUPER ADMIN
              </span>
            </div>
          </div>
      ...
      <main className="flex-1 p-6 max-w-7xl w-full mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          >
            <div>
              <span className="text-2xs font-bold text-slate-500 uppercase tracking-wider">
                Active Verified Facilities
              </span>
              <h3 className="text-2xl font-black text-teal-400 mt-1.5 font-mono">
                {verifiedCount}
              </h3>
            </div>
          ...
          >
            <div>
              ...
              <h3
                className={`text-2xl font-black mt-1.5 font-mono ${pendingCount > 0 ? "text-amber-400 animate-pulse" : "text-slate-400"}`}
              >
                {pendingCount}
              </h3>
            </div>
            <div
              className={`p-3 rounded-xl border ${pendingCount > 0 ? "bg-amber-500/10 border-amber-500/20 text-amber-400" : "bg-slate-800/40 border-slate-800 text-slate-500"}`}
            >
              <Clock size={24} />
            </div>
          </button>
          ...
          >
            <div>
              <span className="text-2xs font-bold text-slate-500 uppercase tracking-wider">
                Unaddressed Support Queries
              </span>
              <h3
                className={`text-2xl font-black mt-1.5 font-mono ${supportTickets.filter((t) => t.status === "pending").length > 0 ? "text-rose-400 animate-pulse" : "text-slate-400"}`}
              >
                {supportTickets.filter((t) => t.status === "pending").length}
              </h3>
            </div>
            <div
              className={`p-3 rounded-xl border ${supportTickets.filter((t) => t.status === "pending").length > 0 ? "bg-rose-500/10 border-rose-500/20 text-rose-455" : "bg-slate-800/40 border-slate-800 text-slate-500"}`}
            >
              <AlertCircle size={24} />
            </div>
          </button>
        ...
        <div className="flex border-b border-slate-850">
          ...
          >
            ...
            {supportTickets.filter((t) => t.status === "pending").length >
              0 && (
              <span className="bg-amber-500/20 text-2xs text-amber-400 font-bold px-1.5 py-0.5 rounded-full border border-amber-500/25 animate-pulse">
                {supportTickets.filter((t) => t.status === "pending").length}
              </span>
            )}
          </button>
          ...
          >
            ...
            {demoRequests.filter((d) => d.status === "pending").length > 0 && (
              <span className="bg-teal-500/20 text-2xs text-teal-400 font-bold px-1.5 py-0.5 rounded-full border border-teal-500/25 animate-pulse">
          ...
          >
            <Tag
              size={12}
              className={
                activeTab === "knowledge" ? "text-teal-400" : "text-slate-400"
              }
            />
          ...
          >
            <Sparkles
              size={12}
              className={
                activeTab === "insights" ? "text-teal-400" : "text-slate-400"
              }
            />
        ...
        {loading ? (
          <div className="bg-slate-900 border border-slate-850 p-10 rounded-2xl text-center space-y-3 shadow-md">
            ...
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">
              Retrieving System Logs & Entities...
            </p>
          </div>
        ) : activeTab === "registry" ? (
          /* REGISTRY PANEL */
            ...
            <div className="p-5 border-b border-slate-850 bg-slate-900/60 flex justify-between items-center">
              <div>
                <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                  Registered Facilities Directories
                </h4>
                <p className="text-2xs text-slate-500 mt-1">
                  Review credentials and toggle verification locks to allow/deny
                  access to tenant portal dashboards.
                </p>
              </div>
            </div>
            ...
            {facilities.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-slate-950/40 text-slate-400 font-bold border-b border-slate-850 text-2xs uppercase">
                      ...
                      >
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <span className="font-bold text-slate-100 block text-xs uppercase">
                                {fac.name}
                              </span>
                              <span className="font-mono text-2xs text-teal-400 font-black block mt-0.5">
                                {fac.code}
                              </span>
                            </div>
                          </div>
                        ...
                        <td className="py-4 px-4 text-slate-450 font-mono text-2xs">
                        ...
                        <td className="py-4 px-4 text-center">
                          ...
                          >
                            <span
                              className={`h-1.5 w-1.5 rounded-full ${fac.is_verified ? "bg-teal-400" : "bg-amber-400"}`}
                            ></span>
                            {fac.is_verified
                              ? "Verified (Unlocked)"
                              : "Awaiting Review"}
                        ...
                        <td className="py-4 px-4 text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button
                              ...
                              className={`inline-flex items-center gap-1.5 font-bold text-2xs uppercase py-1.5 px-3 rounded-lg shadow transition active:scale-[0.98] cursor-pointer ${
                                fac.is_verified
                            ...
                            {fac.is_verified && (
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 font-bold text-2xs uppercase py-1.5 px-3 rounded-lg shadow bg-teal-500 hover:bg-teal-600 text-slate-950 transition active:scale-[0.98] cursor-pointer"
                              >
                                <Building2 size={10} />
          ...
          <div className="bg-slate-900 border border-slate-850 rounded-2xl overflow-hidden shadow-lg space-y-4 p-5 font-sans">
            <div>
              <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                Platform System-Wide Audit Log Stream
              </h4>
              <p className="text-2xs text-slate-500 mt-1">
                Observe real-time security events, administrator configurations,
                and data mutations logged across all clinics.
              </p>
            </div>
            ...
            {auditLogs.length === 0 ? (
              ...
              <div className="space-y-2.5 max-h-125 overflow-y-auto pr-2">
                {auditLogs.map((log) => (
                  <div
                    key={log.id}
                    className="bg-slate-955 border border-slate-855 p-3 rounded-xl flex justify-between items-center gap-4 text-xs font-mono"
                  >
                    <div className="space-y-1.5 flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-2xs text-slate-550">
                          Facility context:{" "}
                          <span className="text-slate-300 font-bold">
                            {log.facility_id || "System"}
                          </span>
                        </span>
                      </div>
                      <p className="text-[10.5px] text-slate-350 truncate">
                        {log.details}
                      </p>
                    </div>
            ...
            <div className="bg-slate-900 border border-slate-850 rounded-2xl overflow-hidden shadow-lg">
              <div className="overflow-x-auto">
                <table className="w-full text-xs text-left text-slate-350">
                  <thead className="bg-slate-955 text-slate-400 font-bold uppercase tracking-wider text-2xs border-b border-slate-850">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-850 bg-slate-900/40">
                    {supportTickets
                      .filter(
                        (t) =>
                          supportFilter === "all" || t.status === supportFilter,
                      )
                      .filter((t) => {
                        ...
                        >
                          ...
                          <td className="px-5 py-4 text-right">
                            <button
                              ...
                              className="bg-slate-800 hover:bg-slate-750 text-teal-400 border border-slate-700 hover:border-teal-500/20 px-3 py-1.5 rounded-lg font-bold text-2xs uppercase transition cursor-pointer"
                            >
                              {ticket.status === "addressed"
                                ? "View Details"
                                : "Address Query"}
                  ...
                  <div className="fixed inset-0 bg-slate-955/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn font-sans">
                    <div className="bg-slate-900 border border-slate-800 w-full max-w-xl rounded-2xl p-6 shadow-2xl space-y-4">
                      <div className="flex justify-between items-start pb-2 border-b border-slate-850">
                        <div>
                          <span className="text-[9px] font-mono text-teal-400 font-bold block">
                            REF: #{ticket.id.substring(7, 13)}
                          </span>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-xs bg-slate-950/45 border border-slate-850 p-3 rounded-xl">
                        <div>
                          <span className="text-[9px] block font-bold text-slate-500 uppercase tracking-wider mb-1">
                            Submitted By
                          </span>
                      ...
                      <div className="bg-slate-950/20 border border-slate-850 p-4 rounded-xl text-xs space-y-2">
                        ...
                        <span className="text-[8px] text-slate-550 block font-mono text-right">
                          {new Date(ticket.created_at).toLocaleString()}
                        </span>
                      </div>
                      ...
                      {ticket.status === "addressed" ? (
                        <div className="bg-green-500/5 border border-green-500/20 p-4 rounded-xl text-xs space-y-2 animate-fadeIn">
                          <span className="text-[9px] font-bold text-green-400 block uppercase">
                            Resolution Response
                          </span>
                        ...
                        <div className="space-y-3 animate-fadeIn">
                          ...
                          <button
                            onClick={() =>
                              handleResolveSupportTicket(ticket.id)
                            }
                ...
                <div className="p-4 border-b border-slate-850 flex justify-between items-center bg-slate-950/25">
                  <div>
                    <h3 className="text-xs font-bold text-slate-100 uppercase tracking-wider">
                      Demo Prospects & Bookings
                    </h3>
                    <p className="text-2xs text-slate-550 mt-0.5">
                      Collect requests from prospective clients, check WhatsApp
                      numbers, and manage follow-ups.
                    </p>
                  </div>
                  <button
                  ...
                  <table className="w-full text-left text-[11px] text-slate-300 font-sans">
                    ...
                    <tbody className="divide-y divide-slate-850/50">
                      {demoRequests.map((prospect) => (
                        <tr
                          key={prospect.id}
                          className="hover:bg-slate-955/10 transition"
                        >
                          ...
                          <td className="py-3 px-4 space-y-1">
                            <a
                              ...
                              className="flex items-center gap-1 text-teal-400 hover:underline hover:text-teal-355 font-bold"
                            >
                          ...
                          <td className="py-3 px-4 text-center">
                            <span className="block font-bold text-slate-205">
                              {prospect.preferred_date}
                            </span>
                            <span className="text-2xs text-slate-500 font-medium">
                              {prospect.preferred_time}
                            </span>
                          </td>
                          <td className="py-3 px-4 text-center text-2xs text-slate-550 font-mono">
                          ...
                          <td className="py-3 px-4 text-right">
                            <div className="flex justify-end gap-1.5">
                              ...
                              {prospect.status === "completed" && (
                                <span className="text-2xs text-slate-600 font-semibold italic">
                                  Archived
                                </span>
                              )}
                            </div>
            ...
            <div className="bg-slate-900 border border-slate-850 rounded-2xl overflow-hidden shadow-lg animate-fadeIn">
              ...
              <div className="p-5 border-b border-slate-850">
                ...
                <p className="text-2xs text-slate-500 mt-0.5">
                  All staff members across all facilities (
                ...
                <table className="w-full text-left text-[11px] text-slate-300 font-sans">
                  ...
                  <tbody className="divide-y divide-slate-850/50">
                    {platformUsers.map((userItem) => {
                      ...
                      const roleBadge =
                        roleColors[userItem.role?.toLowerCase()] ||
                        ...
                        >
                          ...
                          <td className="py-3 px-4">
                            <a
                              ...
                              className="flex items-center gap-1 text-teal-400 hover:underline font-bold"
                            >
                          ...
                          <td className="py-3 px-4">
                            <span
                              className={`px-2 py-0.5 rounded-full border text-[9px] font-bold uppercase tracking-wider ${roleBadge}`}
                            >
                          ...
                          <td className="py-3 px-4 text-center text-slate-500 font-mono text-2xs">
            ...
            <div className="bg-slate-900 border border-slate-850 rounded-2xl overflow-hidden">
              <div className="p-5 border-b border-slate-850 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-2xs text-slate-500">
                    Ranked facility health, support pressure, clinical workflow
                    gaps, and billing exposure across active clients.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateOperationsState({ search: e.target.value })
                      }
                      placeholder="Search client"
                      className="bg-slate-950 border border-slate-800 rounded-lg pl-8 pr-3 py-2 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-teal-500 min-w-47.5"
                    />
                  </div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateOperationsState({ range: e.target.value })
                    }
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateOperationsState({ tier: e.target.value })
                    }
                    className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs font-bold text-slate-300 focus:outline-none focus:ring-1 focus:ring-teal-500"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateOperationsState({ status: e.target.value })
                    }
                    className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs font-bold text-slate-300 focus:outline-none focus:ring-1 focus:ring-teal-500"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateOperationsState({ sort: e.target.value })
                    }
                    className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs font-bold text-slate-300 focus:outline-none focus:ring-1 focus:ring-teal-500"
                  >
                ...
                {[
                  {
                    ...
                    value: operationsSummary.atRisk,
                    icon: ShieldAlert,
                    color: "text-amber-400",
                  },
                  {
                    label: "Open Support",
                    value: operationsSummary.openSupport,
                    icon: MessageSquare,
                    color: "text-blue-400",
                  },
                  {
                    label: "Pending Receivables",
                    value: `Ksh ${Math.round(operationsSummary.pendingReceivables).toLocaleString()}`,
                    icon: CreditCard,
                    color: "text-orange-400",
                  },
                  {
                    label: "Settled Revenue",
                    value: `Ksh ${Math.round(operationsSummary.settledRevenue).toLocaleString()}`,
                    icon: TrendingUp,
                    color: "text-teal-400",
                  },
                ].map((item) => {
                  ...
                  return (
                    <div
                      key={item.label}
                      className="p-4 border-r border-b lg:border-b-0 border-slate-850 last:border-r-0"
                    >
              ...
              <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.6fr)_minmax(340px,0.9fr)]">
                <div className="min-w-0 border-r border-slate-850">
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-240 text-left text-xs border-collapse">
                      <thead>
                        <tr className="bg-slate-950/50 text-slate-450 font-bold border-b border-slate-850 text-2xs uppercase">
                        ...
                        {filteredClientOperationsRows.map((row) => (
                          <tr
                            ...
                            className={`cursor-pointer transition ${
                              selectedOperationsRow?.facilityId ===
                              row.facilityId
                                ? "bg-teal-500/10"
                          ...
                          >
                            <td className="py-3 px-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                  <span className="font-bold text-slate-200 block truncate max-w-55">
                                    {row.facilityName}
                                  </span>
                                  <span className="text-2xs text-slate-500 uppercase font-bold">
                                    {row.code || row.facilityId} ·{" "}
                                    {row.licenseTier}
                                  </span>
                                </div>
                              </div>
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                  <LineChart data={row.trend}>
                                    <Line
                                      type="monotone"
                                      dataKey="visits"
                                      stroke={HEALTH_COLORS[row.healthTone]}
                                      strokeWidth={2}
                                      dot={false}
                                    />
                                  </LineChart>
                            ...
                            <td className="py-3 px-4 text-right">
                              ...
                              <span className="text-2xs text-slate-550 block">
                                pending / {row.orders}
                              </span>
                            </td>
                            <td className="py-3 px-4 text-right">
                            ...
                            <td className="py-3 px-4 text-right">
                              <span
                                className={`font-mono font-bold ${row.openSupport > 0 ? "text-amber-400" : "text-slate-300"}`}
                              >
                                {row.openSupport}
                              </span>
                            </td>
                            <td className="py-3 px-4">
                              <span
                                className={`inline-flex px-2 py-1 rounded-lg border text-2xs font-bold ${
                                  row.healthTone === "critical"
                ...
                <aside className="p-5 space-y-5 bg-slate-950/25">
                  {selectedOperationsRow ? (
                    <>
                      <div className="space-y-3">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <h4 className="text-base font-black text-white mt-1">
                              {selectedOperationsRow.facilityName}
                            </h4>
                            <p className="text-2xs text-slate-500 font-bold uppercase mt-0.5">
                              {selectedOperationsRow.licenseTier} ·{" "}
                              {selectedOperationsRow.verified
                                ? "Verified"
                                : "Locked"}
                            </p>
                          </div>
                          <span
                            className={`text-2xl font-black font-mono ${
                              selectedOperationsRow.healthTone === "critical"
                                ? "text-red-400"
                                : selectedOperationsRow.healthTone === "warning"
                                  ? "text-orange-400"
                                  : selectedOperationsRow.healthTone === "watch"
                                    ? "text-blue-400"
                                    : "text-teal-400"
                            }`}
                          >
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {[
                            ...
                            [
                              "Pending Orders",
                              selectedOperationsRow.pendingOrders,
                            ],
                          ].map(([label, value]) => (
                            <div
                              key={label}
                              className="bg-slate-900 border border-slate-850 rounded-xl p-3"
                            >
                        ...
                        <div className="bg-slate-900 border border-slate-850 rounded-xl p-4 space-y-3">
                          {[
                            [
                              "Triage compliance",
                              selectedOperationsRow.triageComplianceRate,
                            ],
                            [
                              "Lab verification",
                              selectedOperationsRow.labVerificationRate,
                            ],
                            [
                              "Digital payments",
                              selectedOperationsRow.digitalPaymentRate,
                            ],
                          ].map(([label, value]) => (
                            <div key={label} className="space-y-1">
                              <div className="flex justify-between text-2xs font-bold text-slate-400 uppercase">
                                <span>{label}</span>
                                <span>{value}%</span>
                              </div>
                      ...
                      <div className="bg-slate-900 border border-slate-850 rounded-xl p-4 space-y-3">
                        <h5 className="text-2xs font-bold text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        <div className="h-44">
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart
                              data={selectedOperationsRow.trend}
                              margin={{
                                top: 10,
                                right: 10,
                                left: -20,
            ...
            <div className="bg-slate-900 border border-slate-850 p-6 rounded-2xl space-y-4">
              <div>
                ...
                <p className="text-2xs text-slate-550 mt-1">
                  Rule-based checks suggesting training, setup, or workflow
                  interventions across clients.
                </p>
              </div>
                ...
                {getDiagnosticRecommendations().map((rec, idx) => (
                  ...
                  >
                    <div className="space-y-1 flex-1">
                      ...
                      <p className="text-[10.5px] text-slate-400 leading-relaxed pl-3.5">
                        {rec.description}
                      </p>
                    </div>

                    <span
                      className={`text-[9px] font-bold px-2.5 py-1 rounded-lg shrink-0 ${
                        rec.severity === "danger"
