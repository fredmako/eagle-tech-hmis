...
export default function MaternityDashboard({
  ...
  const [wardForm, setWardForm] = useState({
    ...
    name: "",
    description: "",
    drug_store: "INPATIENT",
    consumable_store: "",
    gender: "Female",
    visiting_hours: "12PM-2PM",
    status: "Active",
  });
  ...
  const [regForm, setRegForm] = useState({
    ...
    national_id: "",
    sha_number: "",
    sha_dependent_type: "self",
    sha_status: "unverified",
    nok_name: "",
    nok_phone: "",
    nok_relation: "spouse",
    consent_given: true,
  ...
  const fetchQueue = async () => {
    try {
      ...
      const enriched = (visitsData || []).map((v) => {
        ...
        if (bedsData) {
          assignedBed = bedsData.find(
            (b) =>
              b.description?.includes(v.id) ||
              (b.availability === "Occupied" &&
                b.status === "Active" &&
                b.description === pt?.name),
          );
        }
        return {
          ...v,
    ...
    try {
      ...
      if (blockForm.id) {
        ...
        showNotification?.(
          "success",
          "Block Updated",
          `Maternity block "${payload.name}" updated successfully.`,
        );
      } else {
        const randId = "block_" + Math.random().toString(36).substring(2, 9);
        const { error } = await supabase
          .from("maternity_blocks")
          .insert({ id: randId, ...payload });
        if (error) throw error;
        showNotification?.(
          "success",
          "Block Added",
          `Maternity block "${payload.name}" added successfully.`,
        );
      }
    ...
    try {
      ...
      if (wardForm.id) {
        ...
        showNotification?.(
          "success",
          "Ward Updated",
          `Maternity ward "${payload.name}" updated.`,
        );
      } else {
        ...
        showNotification?.(
          "success",
          "Ward Added",
          `Maternity ward "${payload.name}" added.`,
        );
      }
  ...
  const handleDeleteWard = async (id, name) => {
    if (!window.confirm(`Are you sure you want to delete ward "${name}"?`))
      return;
    try {
      const { error } = await supabase
        .from("maternity_wards")
    ...
    try {
      ...
      if (bedTypeForm.id) {
        const { error } = await supabase
          .from("maternity_bed_types")
          .update(payload)
          .eq("id", bedTypeForm.id);
        if (error) throw error;
        showNotification?.(
          "success",
          "Bed Type Updated",
          `Bed type "${payload.name}" updated.`,
        );
      } else {
  ...
  const handleDeleteBed = async (id, name_code) => {
    if (!window.confirm(`Are you sure you want to delete bed "${name_code}"?`))
      return;
    try {
      const { error } = await supabase
        .from("maternity_beds")
  ...
  const handleRegisterPatient = async (e) => {
    ...
    try {
      ...
      showNotification?.(
        "success",
        "Patient Registered",
        `${regForm.name} successfully registered. Code: ${facilityCode}`,
      );
      setRegForm({
        name: "",
        dob: "",
  ...
  const handleSaveTriage = async (e) => {
    ...
    try {
      ...
      await supabase
        ...
        .eq("id", selectedQueueVisit.id);

      showNotification?.(
        "success",
        "Vitals Recorded",
        `Triage details saved for ${selectedQueueVisit.patient?.name}`,
      );
    ...
    try {
      ...
      const { error: bedErr } = await supabase
        ...
        .update({
          ...
          description:
            selectedAssignVisit.patient?.name +
            ` (Visit: ${selectedAssignVisit.id})`,
        })
        .eq("id", selectedBedId);
      ...
      showNotification?.(
        "success",
        "Bed Assigned",
        `Bed ${bedObj.name_code} assigned to ${selectedAssignVisit.patient?.name}`,
      );
  ...
  const handleDischargePatient = async (visit) => {
    if (!window.confirm(`Discharge ${visit.patient?.name} and release bed?`))
      return;
    setLoading(true);
    try {
      ...
      if (visit.assignedBed) {
        ...
        const myBed = matchedBeds?.find((b) =>
          b.description?.includes(visit.id),
        );
        if (myBed) {
          await supabase
            .from("maternity_beds")
      ...
      showNotification?.(
        "success",
        "Patient Discharged",
        `${visit.patient?.name} discharged. Bed released.`,
      );
      fetchBeds();
      fetchQueue();
    } catch (err) {
    ...
    <div className="flex h-screen bg-slate-950 font-sans overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 bg-slate-900 border-r border-slate-800 flex-col shrink-0">
        ...
        <div className="p-4 border-b border-slate-800 flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-slate-100 text-sm tracking-wider uppercase font-mono">
              HOSI POA
            </div>
            <div className="text-2xs text-teal-400 font-semibold uppercase tracking-widest">
              Maternity Setup
            </div>
          </div>
        </div>
              ...
              <div className="pl-8 pr-1 py-1 space-y-1">
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2 py-1.5 px-2 text-left text-[11px] rounded transition-all ${
                    activeTab === "blocks"
                      ? "text-teal-400 font-bold bg-slate-800/30"
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                  <div className="bg-slate-900 border border-slate-800/80 rounded-xl p-5 flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-2xs uppercase font-bold text-slate-500">
                        Maternity Blocks
                      </div>
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        <h3 className="font-bold text-slate-200 text-sm">
                          Active Maternity Queue
                        </h3>
                        <p className="text-2xs text-slate-500">
                          Admitted or waiting for ANC/Delivery care
                        </p>
                      </div>
                      <button
                        onClick={() => setActiveTab("queue")}
                            ...
                            queue.slice(0, 5).map((v) => (
                              <tr key={v.id} className="hover:bg-slate-800/30">
                                ...
                                <td className="px-4 py-2.5">
                                  {v.patient?.sha_number || "N/A"}
                                </td>
                                <td className="px-4 py-2.5">
                                  {v.assignedBed ? (
                                    <span className="text-teal-400 font-semibold">
                                      {v.assignedBed.name_code}
                                    </span>
                                  ) : (
                                ...
                                <td className="px-4 py-2.5 uppercase font-mono">
                                  {v.priority}
                                </td>
                                <td className="px-4 py-2.5">
                                  <span
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <h2 className="text-md font-bold text-slate-200">
                      Maternity Blocks Setup
                    </h2>
                    <p className="text-2xs text-slate-500">
                      Configure building wings or clinical blocks
                    </p>
                  </div>
                  <button
                    onClick={() => {
            ...
            {activeTab === "wards" && (
              ...
              >
                <div className="flex justify-between items-center">
                  <div>
                    <h2 className="text-md font-bold text-slate-200">
                      Maternity Wards Management
                    </h2>
                    <p className="text-2xs text-slate-500">
                      Configure distinct ward rooms within maternity blocks
                    </p>
                  </div>
                  <button
                    onClick={() => {
                ...
                <div className="bg-slate-900 border border-slate-800/80 rounded-xl overflow-hidden shadow-sm">
                  <table className="w-full text-xs text-left text-slate-400">
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {wards.filter((w) => {
                        const matchBlock =
                          !wardFilterBlock || w.block_name === wardFilterBlock;
                        const matchName =
                          !wardSearchQuery ||
                          w.name
                            .toLowerCase()
                            .includes(wardSearchQuery.toLowerCase());
                          ...
                          .map((w) => (
                            <tr key={w.id} className="hover:bg-slate-800/30">
                              ...
                              <td className="px-4 py-3.5">
                                {w.drug_store || "INPATIENT"}
                              </td>
                              <td className="px-4 py-3.5">
                                {w.consumable_store || "N/A"}
                              </td>
                              ...
                              <td className="px-4 py-3.5">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span
                                    className={`text-2xs font-bold ${w.status === "Active" ? "text-teal-400" : "text-slate-500"}`}
                                  >
                                    {w.status}
                                  </span>
                                </div>
            ...
            {activeTab === "bed_types" && (
              ...
              >
                <div className="flex justify-between items-center">
                  <div>
                    <h2 className="text-md font-bold text-slate-200">
                      Bed Types Classification
                    </h2>
                    <p className="text-2xs text-slate-500">
                      Define general or private bed charges
                    </p>
                  </div>
                  <button
                    onClick={() => {
            ...
            {activeTab === "beds" && (
              ...
              >
                <div className="flex justify-between items-center">
                  <div>
                    <h2 className="text-md font-bold text-slate-200">
                      Beds Management Setup
                    </h2>
                    <p className="text-2xs text-slate-500">
                      Map specific beds to wards and types with billing pricing
                    </p>
                  </div>
                  <button
                    onClick={() => {
                      setBedForm({
                        id: "",
                        name_code: "",
                        bed_type: bedTypes[0]?.name || "",
                        ward_name: wards[0]?.name || "",
                        description: "",
                        availability: "Available",
                        cash_price: "1000",
                        corporate_price: "1000",
                        status: "Active",
                      });
                ...
                <div className="bg-slate-900 border border-slate-800/80 rounded-xl overflow-hidden shadow-sm">
                  <table className="w-full text-xs text-left text-slate-400">
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {beds.filter((b) => {
                        const matchType =
                          !bedFilterType || b.bed_type === bedFilterType;
                        const matchSearch =
                          !bedSearchQuery ||
                          b.name_code
                            .toLowerCase()
                            .includes(bedSearchQuery.toLowerCase()) ||
                          b.ward_name
                            .toLowerCase()
                            .includes(bedSearchQuery.toLowerCase());
                        ...
                        <tr>
                          <td
                            ...
                            className="px-4 py-8 text-center text-slate-500"
                          >
                            No beds matched search filters
                          </td>
                        </tr>
                      ) : (
                        beds
                          ...
                          .map((b) => (
                            <tr key={b.id} className="hover:bg-slate-800/30">
                              ...
                              <td className="px-4 py-3.5 text-slate-500 italic max-w-37.5 truncate">
                                {b.description || "No notes"}
                              </td>
                              <td className="px-4 py-3.5">
                                <span
                                  className={`px-2.5 py-0.5 rounded text-[9px] font-bold font-mono uppercase tracking-wider ${
                              ...
                              <td className="px-4 py-3.5 text-right">
                                <button
                                  onClick={() =>
                                    handleDeleteBed(b.id, b.name_code)
                                  }
            ...
            {activeTab === "registration" && (
              ...
              >
                <div>
                  <h2 className="text-md font-bold text-slate-200">
                    Maternity Department Patient Registration
                  </h2>
                  <p className="text-2xs text-slate-500">
                    Admit a prenatal or maternal patient into isolated maternity
                    records
                  </p>
                </div>
                ...
                <form
                  ...
                  className="bg-slate-900 border border-slate-800/80 rounded-xl p-6 space-y-6"
                >
                  {/* Demographics */}
                  <div className="space-y-4">
                    <h3 className="text-xs font-bold text-teal-400 border-b border-slate-800 pb-2">
                      1. Demographic Profile
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="text-2xs text-slate-400 font-bold uppercase">
                          Patient Full Name
                        </label>
                        <input
                          type="text"
                  ...
                  <div className="space-y-4">
                    <h3 className="text-xs font-bold text-teal-400 border-b border-slate-800 pb-2">
                      2. Social Health Authority (SHA) Coverage
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-1">
                        <label className="text-2xs text-slate-400 font-bold uppercase">
                          SHA Card/Member Number
                        </label>
                        <input
                          type="text"
                      ...
                      <div className="space-y-1">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setRegForm((prev) => ({
                              ...prev,
                              sha_dependent_type: e.target.value,
                            }))
                          }
                          className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs font-medium focus:outline-none focus:border-teal-500/50"
                        >
                      ...
                      <div className="space-y-1">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setRegForm((prev) => ({
                              ...prev,
                              sha_status: e.target.value,
                            }))
                          }
                          className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs font-medium focus:outline-none focus:border-teal-500/50"
                        >
                          <option value="unverified">
                            Unverified (Pending Check)
                          </option>
                          <option value="verified">
                            Verified (Active Cover)
                          </option>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-xs font-bold text-teal-400 border-b border-slate-800 pb-2">
                      3. Maternal & Clinical Indicators
                    </h3>
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setRegForm((prev) => ({
                              ...prev,
                              gravida: parseInt(e.target.value) || 1,
                            }))
                          }
                          className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs font-medium focus:outline-none focus:border-teal-500/50 text-slate-300"
                        />
            ...
            {activeTab === "queue" && (
              ...
              >
                <div>
                  <h2 className="text-md font-bold text-slate-200">
                    Active Inpatient & Maternity Queue
                  </h2>
                  <p className="text-2xs text-slate-500">
                    Perform admissions triage vitals check-ins and bed
                    configurations
                  </p>
                </div>
                        ...
                        queue.map((v) => {
                          ...
                          return (
                            <tr key={v.id} className="hover:bg-slate-800/30">
                              ...
                              <td className="px-4 py-3.5 uppercase font-bold text-[9px] font-mono tracking-wider">
                                {v.priority}
                              </td>
                              <td className="px-4 py-3.5">
                                {v.assignedBed ? (
            ...
            {activeTab === "inventory" && (
              ...
              >
                <div>
                  <h2 className="text-md font-bold text-slate-200">
                    Maternity Supplies & Drug Stock
                  </h2>
                  <p className="text-2xs text-slate-500">
                    Monitor active pharmaceutical stocks and neonatal kit
                    supplies
                  </p>
                </div>
                      ...
                      {inventory.length === 0 ? (
                        ...
                        inventory.map((item) => (
                          <tr key={item.id} className="hover:bg-slate-800/30">
                            ...
                            <td className="px-5 py-3.5 text-slate-400 uppercase text-2xs tracking-wider">
                              {item.category}
                            </td>
                            <td
            ...
            {activeTab === "reports" && (
              ...
              >
                <div className="flex justify-between items-center pb-2 border-b border-slate-800">
                  <div>
                    <h2 className="text-md font-bold text-slate-200">
                      Outcome Analysis & ANC Stats
                    </h2>
                    <p className="text-2xs text-slate-500">
                      View real-time department statistics and health outcomes
                    </p>
                  </div>
                  <button
                    type="button"
                  ...
                  <div className="bg-slate-900 border border-slate-800/80 rounded-xl p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2">
                      <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 text-center">
                        <div className="text-2xs text-slate-500 uppercase font-bold">
                          Total Deliveries
                        </div>
                      ...
                      <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 text-center">
                        ...
                        <div className="text-3xl font-mono font-bold text-teal-400 mt-1">
                          35
                        </div>
                      </div>
                      <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 text-center">
      ...
      {showBlockModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div
            initial={{ scale: 0.95 }}
            animate={{ scale: 1 }}
            className="bg-slate-900 border border-slate-850 rounded-xl max-w-md w-full p-6 space-y-4"
          >
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="font-bold text-slate-100 text-sm">
                {blockForm.id ? "Edit Block" : "Add Maternity Block"}
              </h3>
              <button
            ...
            <form onSubmit={handleSaveBlock} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full py-2 bg-teal-600 hover:bg-teal-500 rounded text-xs font-bold text-white transition-all uppercase tracking-wide"
              >
      ...
      {showWardModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div
            initial={{ scale: 0.95 }}
            animate={{ scale: 1 }}
            className="bg-slate-900 border border-slate-850 rounded-xl max-w-lg w-full p-6 space-y-4"
          >
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="font-bold text-slate-100 text-sm">
                {wardForm.id ? "Edit Ward" : "Add Maternity Ward"}
              </h3>
              <button
            ...
            <form onSubmit={handleSaveWard} className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setWardForm((prev) => ({
                      ...prev,
                      consumable_store: e.target.value,
                    }))
                  }
                  className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs font-semibold focus:outline-none focus:border-teal-500/50"
                />
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setWardForm((prev) => ({ ...prev, gender: e.target.value }))
                  }
                  className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs font-semibold focus:outline-none focus:border-teal-500/50"
                >
              ...
              <div className="col-span-2 pt-2">
                <button
                  type="submit"
                  className="w-full py-2.5 bg-teal-600 hover:bg-teal-500 rounded text-xs font-bold text-white transition-all uppercase tracking-wide"
                >
      ...
      {showBedTypeModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="font-bold text-slate-100 text-sm">
                {bedTypeForm.id ? "Edit Bed Type" : "Add Bed Type"}
              </h3>
              <button
      ...
      {showBedModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="font-bold text-slate-100 text-sm">
                {bedForm.id ? "Edit Bed" : "Add Bed Code"}
              </h3>
              <button
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBedForm((prev) => ({
                      ...prev,
                      corporate_price: e.target.value,
                    }))
                  }
                  className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs font-semibold focus:outline-none focus:border-teal-500/50"
                  placeholder="e.g. 1000"
          ...
          >
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-100 text-sm">
                  Admissions Triage: {selectedQueueVisit?.patient?.name}
                </h3>
              </div>
              <button
                onClick={() => {
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTriageForm((prev) => ({ ...prev, spo2: e.target.value }))
                  }
                  className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs font-semibold focus:outline-none focus:border-teal-500/50"
                  placeholder="98"
                />
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  {beds
                    .filter(
                      (b) =>
                        b.availability === "Available" && b.status === "Active",
                    )
                    .map((b) => (
                      <option key={b.id} value={b.id}>
                        {b.name_code} - {b.bed_type} ({b.ward_name}) - Ksh{" "}
                        {b.cash_price}
                      </option>
                    ))}
                </select>
